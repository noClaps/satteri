...
//!
//! Implemented by both `Arena` (owned) and `ArenaView<'_>` (zero-copy view
...
impl ReadArena for crate::arena::Arena {
    fn get_node(&self, node_id: u32) -> &ArenaNode { self.get_node(node_id) }
    fn get_children(&self, node_id: u32) -> &[u32] { self.get_children(node_id) }
    fn get_type_data(&self, node_id: u32) -> &[u8] { self.get_type_data(node_id) }
    fn get_str(&self, string_ref: StringRef) -> &str { self.get_str(string_ref) }
    fn source(&self) -> &str { self.source() }
    fn len(&self) -> usize { self.len() }
}

impl ReadArena for crate::raw_buffer::ArenaView<'_> {
    fn get_node(&self, node_id: u32) -> &ArenaNode { self.get_node(node_id) }
    fn get_children(&self, node_id: u32) -> &[u32] { self.get_children(node_id) }
    fn get_type_data(&self, node_id: u32) -> &[u8] { self.get_type_data(node_id) }
    fn get_str(&self, string_ref: StringRef) -> &str { self.get_str(string_ref) }
    fn source(&self) -> &str { self.get_source() }
    fn len(&self) -> usize { self.node_count() as usize }
}
