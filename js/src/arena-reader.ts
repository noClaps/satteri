...
export class ArenaReader {
  ...
  get nodeCount(): number { return this.#header.nodeCount; }
  get header(): BufferHeader { return { ...this.#header }; }
   ...
   */
  getDefinitionData(nodeId: number): { url: string; title: string | null; identifier: string; label: string } {
    const data = this.getTypeData(nodeId);
    const urlRef = this.readStringRef(data, 0);
    const titleRef = this.readStringRef(data, 8);
